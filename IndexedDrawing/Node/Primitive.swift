...
class Primitive: Node {
    ...
    var indexBuffer_vertex: MTLBuffer!
    // BufferData
    var vertices: [Vertex]!
    var indices: [UInt16]!
    var indices_vertex: [UInt16]!
    // States
    ...
    private func buildBuffers(device: MTLDevice) {
        ...
        indexBuffer = device.makeBuffer(bytes: indices,
                                        ...
                                        options: [])
        indexBuffer_vertex = device.makeBuffer(bytes: indices_vertex,
                                  length: MemoryLayout<UInt16>.stride * indices_vertex.count,
                                  options: [])
    }
    ...
    override func render(commandEncoder: MTLRenderCommandEncoder, deltaTime: Float) {
        ...
        commandEncoder.drawIndexedPrimitives(type: .point,
                                             indexCount: indices_vertex.count * 2,
                                             indexType: .uint16,
